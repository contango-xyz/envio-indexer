...
import { ContangoEvents, EventType, GenericEvent, MigratedEvent, TransferEvent } from "../utils/types";
...
import { AccountingType, allocateFundingCostToLots, allocateFundingProfitToLots, handleCostDelta, handleSizeDelta } from "./lotsAccounting";
...
const eventsToFillItem = (position: Position, debtToken: Token, collateralToken: Token, event: Exclude<ContangoEvents, ERC20_Transfer_event>, existingFillItem: PartialFillItem): PartialFillItem => {
...
export const eventsReducer = async (maybeLastEventInTx: GenericEvent & { positionId: string }, context: handlerContext) => {
  ...
  context.log.debug(`Processing events for positionId: ${positionId} length: ${events.length}`)
  ...
  if (partialFillItem.tradePrice_long === 0n || partialFillItem.tradePrice_short === 0n) {
    const markPrice = await getMarkPrice({ chainId, positionId: positionSnapshot.positionId as Hex, blockNumber, context })
  ...
  const newFillItem = {
    ...
    cashflowToken_id: fillItem.cashflowToken_id,
